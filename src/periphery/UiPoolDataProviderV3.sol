...
import {Ownable} from "src/dependencies/openzeppelin/contracts/Ownable.sol";
...
import {UserConfiguration} from "src/protocol/libraries/configuration/UserConfiguration.sol";
...
contract UiPoolDataProviderV3 is IUiPoolDataProviderV3, Ownable {
    ...
    function setTokenName(address asset, string memory name, string memory symbol) external onlyOwner {
        tokenNames[asset] = name;
        tokenSymbols[asset] = symbol;
    }
        ...
        returns (AggregatedReserveData[] memory, BaseCurrencyInfo memory)
    {
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            reserveData.liquidityIndex = uint128(pool.getReserveNormalizedIncome(reserves[i]));
            //variable borrow index. Expressed in ray
            reserveData.variableBorrowIndex = uint128(pool.getReserveNormalizedVariableDebt(reserves[i]));
            ...
            if (!baseData.configuration.getVirtual()) {
            ...
            if (baseData.configuration.getVirtual()) {
                reserveData.symbol = tokenSymbols[reserveData.underlyingAsset];
                reserveData.name = tokenNames[reserveData.underlyingAsset];
            } else {
            ...
            reserveData.isVirtual = reserveConfigurationMap.getVirtual();
        ...
        try oracle.BASE_CURRENCY_UNIT() returns (uint256 baseCurrencyUnit) {
            ...
            baseCurrencyInfo.marketReferenceCurrencyUnit = 10 ** marketReferenceCurrencyPriceInUsdProxyAggregator.decimals();
            baseCurrencyInfo
