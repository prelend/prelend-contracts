...
library LiquidationLogic {
    ...
    function executeLiquidationCall(
        ...
        vars.debtLiquidityIndex = debtReserve.getNormalizedIncome();
        ...
        if (debtReserve.configuration.getVirtual()) {
            IAToken(debtReserve.aTokenAddress).transferFrom(
                msg.sender,
                address(this),
                vars.actualDebtToLiquidate
            );
            IAToken(debtReserve.aTokenAddress).burn(
                ...
                vars.actualDebtToLiquidate,
                vars.debtLiquidityIndex
            );
        } else {
            // Transfers the debt asset being repaid to the aToken, where the liquidity is kept
            IERC20(toActualAsset[params.debtAsset]).safeTransferFrom(
                msg.sender,
    ...
    function _burnCollateralATokens(
        ...
        require(!collateralReserve.configuration.getVirtual(), "virtual asset");
