...
library ReserveLogic {
    ...
    function getNormalizedIncome(
        ...
        if (reserve.rebaseTracker != address(0)) {
            (uint256 l, uint256 p) = IRebaseTracker(reserve.rebaseTracker)
                .peek();
            index = Math.mulDiv(index, p, l);
        }
    ...
    function init(
        ...
        reserve.rebaseTracker = rebaseTracker;
    }
        ...
        (
            ...
            .calculateInterestRates(
                DataTypes.CalculateInterestRatesParams({
                    ...
                    reserveBalance: reserveCache
                        .reserveConfiguration
                        .getVirtual()
                        ? 0
                        : IERC20(reserveAddress).balanceOf(
    ...
    function _updateIndexes(
        ...
        address rebaseTracker = reserve.rebaseTracker;
        uint256 rebaseIndexStored;
        uint256 rebaseIndex;
        if (rebaseTracker != address(0)) {
            (rebaseIndexStored, rebaseIndex) = IRebaseTracker(rebaseTracker)
                .catchUp();
        }

        if (
            reserveCache.currLiquidityRate != 0 ||
            rebaseIndex != rebaseIndexStored
        ) {
            ...
            if (rebaseIndex != rebaseIndexStored) {
                reserveCache.nextLiquidityIndex = Math.mulDiv(
                    reserveCache.nextLiquidityIndex,
                    rebaseIndex,
                    rebaseIndexStored
                );
        ...
        if (
            ...
            if (rebaseIndex != rebaseIndexStored) {
                reserveCache.nextVariableBorrowIndex = Math.mulDiv(
                    reserveCache.nextVariableBorrowIndex,
                    rebaseIndex,
                    rebaseIndexStored
                );
        ...
        //rebasing for stable debts is not implemented
    }
