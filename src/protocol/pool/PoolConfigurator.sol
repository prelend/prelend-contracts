...
contract PoolConfigurator is VersionedInitializable, IPoolConfigurator {
  ...
  function setReserveRebaseTrackerAddress(
    address asset,
    address newRebaseTrackerAddress
  ) external onlyRiskOrPoolAdmins {
    DataTypes.ReserveData memory reserve = _pool.getReserveData(asset);
    address oldRebaseTracker = reserve.rebaseTracker;
    _pool.setReserveRebaseTrackerAddress(asset, newRebaseTrackerAddress);
    emit ReserveRebaseTrackerChanged(asset, oldRebaseTracker, newRebaseTrackerAddress);
  }
